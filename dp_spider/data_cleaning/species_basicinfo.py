...
# === 全局配置 ===
# 输入目录：存放原始 JSON 文件
INPUT_DIR = 'data/species_basicinfo'
# 输出目录：存放清洗后的 CSV 文件
OUTPUT_DIR = 'cleaned_data/species_basicinfo'

# 创建输出目录（如果不存在）
os.makedirs(OUTPUT_DIR, exist_ok=True)

# 主表字段映射关系（species_basic_info）
BASIC_INFO_FIELDS = {
    'SC_GUID': 'species_guid',
    'TP_GUID': 'record_guid',
    'SSNameSci': 'scientific_name',
    'SEName': 'english_name',
    'BiologicalProperties': 'biological_properties',
    'MorphologicalCharacteristics': 'morphological_characteristics',
    'DetectionMethod': 'detection_method',
    'DistributionDescription': 'distribution_description',
    'ICodeID': 'icode_id',
    'ICodeName': 'icode_name',
    'Page': 'page',
    'Remark': 'remark',
    'TP_AUTHOR': 'author',
...
def process_basic_info_record(record):
    """
    处理单个物种基本信息记录，映射到主表字段，并进行数据类型转换。
    ...
    basic_info_record = {
        BASIC_INFO_FIELDS.get(key, key): record.get(key, '')
        for key in BASIC_INFO_FIELDS
    }
    # 添加 id 字段，设为 None，由数据库自增
    basic_info_record['id'] = None

    # 处理日期字段，确保符合 DATETIME 格式
    date_fields = ['created_time', 'update_time', 'temp_created_time']
    for field in date_fields:
        if basic_info_record[field]:
            basic_info_record[field] = parse_datetime(basic_info_record[field])
        else:
            basic_info_record[field] = None

    return basic_info_record


def process_reference_record(record):
    """
    处理物种基本信息记录中的引用信息 (cankao)，映射到引用表字段。
...
# === 主处理逻辑 ===
def clean_species_basic_info_data():
    """
    主函数：清洗 species_basicinfo 目录下的所有 JSON 文件，并保存为 CSV。
    """
    # 遍历输入目录下的所有 JSON 文件
    for filename in os.listdir(INPUT_DIR):
        if filename.startswith('species_basicinfo_batch_') and filename.endswith('.json'):
            ...
            for record in data:
                # 处理主表数据
                basic_info_record = process_basic_info_record(record)
                basic_info_data.append(basic_info_record)
            ...
            # 保存主表 CSV 文件
            basic_info_df.to_csv(
                os.path.join(OUTPUT_DIR, f'species_basic_info_batch_{batch_num}.csv'),
                index=False,
            ...
            print(f"完成处理文件: {filename}，生成 CSV 文件: "
                  f"species_basic_info_batch_{batch_num}.csv 和 "
