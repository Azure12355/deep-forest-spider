# data_transfer/species_other_names_import.py
import os
import csv
import logging
...
logging.basicConfig(
    ...
    datefmt='%Y-%m-%d %H:%M:%S',
    handlers=[
        logging.FileHandler('log/import_species_other_names.log'),
...
class SpeciesOtherNamesImporter:
    def __init__(self):
        # 数据库连接配置
        self.db_config = {
            'host': 'localhost',
    ...
    def check_table_exists(self):
        """检查目标表是否存在"""
        try:
            self.cursor.execute("SHOW TABLES LIKE 'species_other_names'")
            result = self.cursor.fetchone()
            if result:
                logger.info("表 species_other_names 已确认存在")
                return True
            else:
                logger.error("表 species_other_names 不存在")
                return False
        except pymysql.Error as e:
    ...
    def find_csv_files(self, base_dir='../cleaned_data/meta_info_list'):
        """查找所有other_names_batch开头的CSV文件"""
        csv_files = []
        for file in os.listdir(base_dir):
            if file.startswith('other_names_batch') and file.endswith('.csv'):
    ...
    def process_csv_file(self, file_path):
        ...
        try:
            ...
            for row in tqdm(rows, desc=f"处理 {os.path.basename(file_path)}"):
                ...
                try:
                    # 检查必填字段
                    if not row.get('species_guid') or not row.get('other_name'):
                        logger.warning(f"跳过缺失必填字段(species_guid/other_name)的行: {row}")
                        self.failed_rows += 1
                        continue
                    ...
                    insert_data = {
                        ...
                        'other_name_type': row.get('other_name_type'),
                        'named_year': row.get('named_year'),
                    ...
                    sql = f"INSERT INTO species_other_names ({columns}) VALUES ({placeholders})"
...
if __name__ == '__main__':
    importer = SpeciesOtherNamesImporter()
    importer.run_import()
